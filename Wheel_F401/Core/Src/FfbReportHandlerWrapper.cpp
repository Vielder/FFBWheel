...
extern "C" {
	...
	extern "C" void callFfbOnCreateNewEffect(void *handler, FFB_CreateNewEffect_Feature_Data_t *inData) {
	...
	extern "C" TEffectState_c* callGetEffectData(void *handler, uint8_t id) {
		return (TEffectState_c*) static_cast<FfbReportHandler*>(handler)->getEffectData(
				id);
	}
